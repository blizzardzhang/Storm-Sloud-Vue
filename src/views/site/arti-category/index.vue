<template>
  <div class="container">
    <Breadcrumb :items="['menu.site', 'menu.site.artiCategory']" />
    <a-card class="general-card" :title="$t('site.artiCategory.list')">
      <!--筛选条件-->
      <a-row>
        <a-col :flex="1">
          <a-form
          ...
          >
            <a-row :gutter="8">
              <a-col :span="12">
                <a-form-item filed="type" :label="$t('artiCategory.table.type')">
                  <a-select
                    v-model="formModel.type"
                    allow-clear
                    :placeholder="$t('artiCategory.table.type.placeholder')"
                  >
              ...
              <a-col :span="12">
                <a-form-item filed="name" :label="$t('artiCategory.table.name')">
                  <a-input
                    v-model="formModel.name"
                    :placeholder="$t('artiCategory.table.name.placeholder')"
                  />
                </a-form-item>
              </a-col>
            </a-row>
...
<!--          如果type =1 显示技术 type =2 显示生活-->
    ...
    <a-modal
     ...
     :title="
      isEdit
      ? $t('site.artiCategory.modal.title.edit')
      : $t('site.artiCategory.modal.title.create')
    "
    >
      <a-form
        ref="formRef"
      ...
      >
        <a-form-item
          field="type"
          :label="$t('artiCategory.table.type') "
        ...
        <a-form-item
          field="name"
          :label="$t('artiCategory.table.name') "
          validate-trigger="blur"
          required
        >
          <a-input
...
import { deleteArtiCategory, getArtiCategoryPage, saveOrUpdateArtiCategory } from '@/api/site/arti-category.ts'
...
const columns: TableColumnData[] = [
  {
    title: t("artiCategory.table.type"),
    dataIndex: "type",
    fixed: "left",
    width: 120,
    slotName: "type",
  ...
  {
    title: t("artiCategory.table.name"),
    dataIndex: "name",
    width: 100,
  },
  {
...
const fetchData = async(
  ...
  setLoading(true);
  try{
    const res = await getArtiCategoryPage(params);
    if (res.code === 200){
...
const handleDelete = async () => {
  ...
  try {
    ...
    const res = await deleteArtiCategory(params);
    if (res.code === 200) {
...
const rules = {
  type: [{ required: true, message: t("artiCateGory.rules.type.required") }],
  name: [{ required: true, message: t("artiCateGory.rules.name.required") }],
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await saveOrUpdateArtiCategory(data);
    if (res.code === 200) {
      Message.success(t("common.success"));
    } else {
      Message.error(t("common.fail"));
